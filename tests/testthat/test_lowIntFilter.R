context("DGEtools - tests for lowIntFilter.R functions")
...
test_that('lowIntFilter: lowIntFilter()', {
    ...
    lowIntFilter_three_test <- lowIntFilter(DGEobj1, fpkThreshold = 5)

    expect_s3_class(lowIntFilter_three_test, "DGEobj")
    expect_equal(nrow(lowIntFilter_three_test$counts), 11679)

    lowIntFilter_four_test <- lowIntFilter(DGEobj1, countThreshold = 10, zfpkmThreshold = -3.0, sampleFraction = 0.75)
...
test_that('lowIntFilter: incorrect usage', {
    ...
    expect_error(lowIntFilter(DGEobj1, zfpkmThreshold = 3.0, tpmThreshold = 1),
