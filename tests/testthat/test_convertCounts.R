context("DGEtools - tests for convertCounts.R functions")
...
test_that("convertCounts.R: convertCounts()", {
    ...
    expect_true("matrix" %in% class(count_matrix))
    expect_identical(dim(DGEobj1$counts), dim(count_matrix))
    expect_error(convertCounts(counts = DGEobj1$counts, unit = "TPM"), regexp = "geneLength is required for unit = FPK|FPKM|TPM")
    expect_warning(convertCounts(counts = DGEobj1$counts, unit = "TPM", geneLength = genelength, normalize = "TMM"),
                   regexp = "TPM normalization overides TMM normalization!")
    expect_warning(convertCounts(counts = DGEobj1$counts, unit = "TPM", geneLength = genelength, prior.count = 1,log = TRUE),
                   regexp = "Using a prior.count for logTPM calculations is not recommended and may produce unpredictable results!")

    warnings_tpm <- capture_warnings(convertCounts(counts = DGEobj1$counts_orig[1:100,], unit = "TPM", geneLength  = genelength[1:100]))
...
test_that("convertCounts.R: tpm.direct()", {
    ...
    expect_error(tpm.direct(DGEobj1$counts, geneLength = as.data.frame(genelength)),
                 regexp = "The dimensions of counts and geneLength should match.")

    # testing collapse parameter
    tpmObj <- tpm.direct(DGEobj1$counts, geneLength = as.matrix(genelength), collapse = TRUE)
    expect_true("matrix" %in% class(tpmObj))
})
