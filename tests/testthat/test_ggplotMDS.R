context("DGEtools - tests for ggplotMDS.R functions")


test_that("ggplotMDS.R: ggplotMDS()", {
    skip_if(is.null(DGEobj1$DGEList))
    skip_if(is.null(DGEobj1$design$Barcode))
    ...
    mds_plot <- ggplotMDS(DGEdata   = DGEobj1,
                          colorBy   = DGEobj1$design$Barcode,
                          sizeBy    = DGEobj1$design$Dose..mpk.,
                          shapeBy   = DGEobj1$design$Species,
    ...
    mds_plot <- ggplotMDS(DGEdata = DGEobj1,
                          colorBy = DGEobj1$design$Species,
                          colors  = c("green"))
    expect_s3_class(mds_plot$plot, c("gg", "ggplot"))
    ...
    expect_error(ggplotMDS(DGEdata = DGEobj1),
...
test_that("ggplotMDS.R: MDS_var_explained()", {
    ...
    # testing mds matrix
    var_result <- MDS_var_explained(DGEobj1$counts)
    expect_length(var_result, 3)
    expect_named(var_result, c("varexp", "cumvar", "var_explained"))
    expect_type(var_result, "list")
    expect_s3_class(var_result$varexp, c("gg", "ggplot"))
