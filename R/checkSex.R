...
checkSex <- function(dgeObj,
    ...
    if (orig) {
        log2CPM <- convertCounts(DGEobj::getItem(dgeObj, "counts_orig"), unit = "cpm", log = TRUE, normalize = "tmm")
    } else {
        log2CPM <- convertCounts(DGEobj::getItem(dgeObj, "counts"), unit = "cpm", log = TRUE, normalize = "tmm")
    }
